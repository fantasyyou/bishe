...
def kaishi(path):
    ...
    for filename in os.listdir(test_images_dir):
        ...
        converet(filename)
        tf.reset_default_graph()#重置tensorflow的旧变量，重置图标
        bank_card=distinguish(filename,test_images_dir)
        tf.reset_default_graph()#重置tensorflow的旧变量，重置图标
        return bank_card;

# ***************************************     动态创建文件和删除文件    forecast   ***************************
def mkdir(path):
    # 去除首位空格
    path = path.strip()
    # 去除尾部 \ 符号
...
# ***************************************     删除指定的字符    forecast   ***************************
def delete_specified_character(my_str):
...
def handle_client(client_socket,user_id):
    ...
    处理客户端请求
    """
    path='test_images/' + user_id + '/'
    mkdir(path)
    request_data = client_socket.recv(1024*1024*1024)
    with open(path+user_id+'.jpg', 'wb') as fp:
        fp.write(request_data)
    with open('test_images/'+user_id+'.jpg', 'wb') as fp:
        fp.write(request_data)

    try:
        bank_card = kaishi(path)
    except FileNotFoundError:
        shutil.rmtree(path)
        os.remove('test_images/' + user_id + '.jpg')
        client_socket.send(bytes("识别失败", "utf-8"))
    else:
        shutil.rmtree(path)
        os.remove('test_images/'+user_id+'.jpg')
        # 向客户端返回响应数据
        client_socket.send(bytes(bank_card, "utf-8"))
        # 关闭客户端连接
        client_socket.close()
...
if __name__ == "__main__":
    ...
    while True:
        ...
        user_id = str(client_address)
        user_id = delete_specified_character(user_id)
        handle_client_process = Process(target=handle_client, args=(client_socket,user_id))
